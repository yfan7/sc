    ...
    def p_sample(
        ...
        if conf.inpa_inj_sched_prev:
            ...
            if pred_xstart is not None:
                ...
                # save_interm =   (1 - gt_keep_mask) * x
                # Image.fromarray(save_interm.cpu().numpy()[0][0]).save("temps/it.png", "PNG")
                ...
                print('sum gt_keep_mask',gt_keep_mask.shape,gt_keep_mask.max(),gt_keep_mask.min())
        if gt_keep_mask is None:
            gt_keep_mask = th.zeros_like(x)
        ...
        if len(gt_keep_mask.shape)<4:
            stacked_gt_mask = th.stack((gt_keep_mask,gt_keep_mask,gt_keep_mask),dim=1)
        else:
            stacked_gt_mask = gt_keep_mask
        print('sum stacked_gt_mask',stacked_gt_mask.sum())

        sample = out["mean"] + nonzero_mask * \
            th.exp(0.5 * out["log_variance"]) * noise
        print('sum sample before',th.sum(sample*(1-stacked_gt_mask)))
        
        # sample = th.where(stacked_gt_mask != 0, sample / 127.5 - 1, sample)
        print('sum sample after',th.sum(sample*(1-stacked_gt_mask)))
            ...
            for t_last, t_cur in time_pairs:
                ...
                if t_cur < t_last:  # reverse
                    with th.no_grad():
                        ...
                        Image.fromarray(np.uint8(image_after_step.permute(0,2,3,1).detach().cpu().numpy().squeeze()), mode='RGB').save("temps/it.png", "PNG")
