...
def random_bbox():
    ...
    """
    img_height, img_width, _ = [32,32,3]
    h, w = [16,16]
...
def bbox2mask(bboxes, height, width, max_delta_h, max_delta_w):
    # channel = bboxes.size(-1)
    mask = np.zeros((1, height, width)).astype(np.float32)
    bbox = bboxes[0]
...
def mask_image(x, bboxes):
    height, width, _ = [32,32,3]
    max_delta_h, max_delta_w = [3, 3]
    mask = bbox2mask(bboxes, height, width, max_delta_h, max_delta_w)
    # result = x * (1. - mask)
    # invert_result = x * mask
    result = x * mask
    result[1,:,:] = x[1,:,:]
    invert_result = x * (1-mask)
    return result, mask, invert_result
