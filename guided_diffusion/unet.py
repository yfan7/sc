...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb):
        ...
        assert torch.isfinite(emb_out).all()
...
class FFParser(nn.Module):
    def __init__(self, dim,train_dim,d=20, h=128, w=65):
        super().__init__()
        # print('h in FFParser line 457',h)
        # print('w in FFParser line 457',w)
        self.train_dim = train_dim
        if self.train_dim ==3:
            self.complex_weight = nn.Parameter(torch.randn(dim,d, h, w, 2, dtype=torch.float32) * 0.02)
        else:
    ...
    def forward(self, x, spatial_size=None):
        if self.train_dim ==3:
            ...
            # print('x in FFParser line 475',x.shape)
            # print('weight in FFParser line 475',weight.shape)
            x = x * weight
            x = torch.fft.irfft2(x, s=( H, W), dim=( 3, 4), norm='ortho')
            
            x = x.reshape(B, C, D, H, W).to(torch.float32)
            assert torch.isfinite(x).all()
        elif self.train_dim ==2:
            B, C, H, W = x.shape
            assert H == W, "height and width are not equal"
            if spatial_size is None:
                a = b = H
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if high_way:
            ...
            if dims ==3:
                ...
                print('--in UNet model neither 3d or 2d, ERROR--')
            if self.sc:
                self.hwm = Generic_UNet(False,dims,self.in_channels - 3, features,3, 4,self.image_size,conv_op = conv_op,norm_op = norm_op, dropout_op = dropout_op)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        h = x.type(self.dtype)
        print('initial h size',h.shape)
        # print('unet line 781 self.dtype',self.dtype)
        assert torch.isfinite(h).all()
        if self.sc:
            c = h[:,:3,...]
        else:
        ...
        # print('------len hs----------',len(hs))
        if self.sc:
            uemb, cal = self.highway_forward(c, [hs[3],hs[6],hs[8]])
        else:
            uemb, cal = self.highway_forward(c, [hs[3],hs[6],hs[9],hs[12]])
        assert torch.isfinite(uemb).all()
        assert torch.isfinite(cal).all()
        h = h + uemb
...
class PredictionNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if regions_class_order is None:
            predicted = aggregated_results.argmax(0)
        else:
            if all_in_gpu:
                class_probabilities_here = aggregated_results.detach().cpu().numpy()
            else:
                class_probabilities_here = aggregated_results
            predicted = np.zeros(class_probabilities_here.shape[1:], dtype=np.float32)
            for i, c in enumerate(regions_class_order):
                predicted[class_probabilities_here[i] > 0.5] = c
        ...
        return predicted, aggregated_results
    ...
    def _internal_predict_2D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
        ...
        if regions_class_order is None:
            predicted = predicted_probabilities.argmax(0)
            predicted = predicted.detach().cpu().numpy()
            predicted_probabilities = predicted_probabilities.detach().cpu().numpy()
        else:
            predicted_probabilities = predicted_probabilities.detach().cpu().numpy()
            predicted = np.zeros(predicted_probabilities.shape[1:], dtype=np.float32)
            for i, c in enumerate(regions_class_order):
                predicted[predicted_probabilities[i] > 0.5] = c

        return predicted, predicted_probabilities
    ...
    def _internal_predict_2D_2Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if regions_class_order is None:
            predicted = class_probabilities.argmax(0)
        else:
            if all_in_gpu:
        ...
        if verbose: print("prediction done")
        return predicted, class_probabilities
    ...
    def _internal_predict_3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
        ...
        for s in range(x.shape[1]):
            ...
            predicted.append(pred[None])
            softmax_pred.append(softmax_pres[None])
        predicted = np.vstack(predicted)
        softmax_pred = np.vstack(softmax_pred).transpose((1, 0, 2, 3))
        return predicted, softmax_pred
    ...
    def _internal_predict_3D_2Dconv_tiled(self, x: np.ndarray, patch_size: Tuple[int, int], do_mirroring: bool,
        ...
        for s in range(x.shape[1]):
            pred, softmax_pres = self._internal_predict_2D_2Dconv_tiled(
...
class Generic_UNet(PredictionNetwork):
    ...
    def __init__(self, train_dim,input_channels, base_num_features, num_classes, num_pool, image_size, num_conv_per_stage=2,
        ...
        if conv_op == nn.Conv2d:
            ...
            if pool_op_kernel_sizes is None:
                pool_op_kernel_sizes = [(2, 2)] * (num_pool-2)
                pool_op_kernel_sizes += [(1,1)] *2
        ...
        for d in range(num_pool):
            ...
            if d < num_pool -1:
                self.conv_trans_blocks_a.append(conv_nd(self.train_dim, self.mult[d] * 64, 2 **(d+5), 1))
                self.conv_trans_blocks_b.append(conv_nd(self.train_dim, 2 **(d+5), 1, 1))
                
                
            # if d != num_pool - 1:
            #     # print('line 2067',self.image_size // (2 **(d+1)))
            #     if self.train_dim==3:
            #         self.ffparser.append(FFParser(output_features,self.train_dim,d=3, h=self.image_size // (2 **(d+1)), w=self.image_size // (2 **(d+2))+1))
            #     elif self.train_dim==2:
            #         self.ffparser.append(FFParser(output_features,self.train_dim, h=self.image_size // (2 **(d+1)), w=self.image_size // (2 **(d+2))+1))
            #     else:
            #         print('ffparser train_dim not in (2,3), ERROR')
            if d < num_pool:
        ...
        if self._deep_supervision:
            for ds in range(len(self.conv_blocks_localization)):
                self.pred_outputs.append(conv_op(self.conv_blocks_localization[ds][-1].output_channels, num_classes,
                                                1, 1, 0, 1, 1, pred_output_use_bias))
        else:
            self.pred_outputs.append(conv_op(self.conv_blocks_localization[-1][-1].output_channels, num_classes,
                            1, 1, 0, 1, 1, pred_output_use_bias))
        ...
        self.pred_outputs = nn.ModuleList(self.pred_outputs)
        if self.upscale_logits:
    ...
    def forward(self, x, hs):
        ...
        # print('-------- in Generic Unet forward ----------')
        # GPUtil.showUtilization()
        skips = []
        pred_outputs = []
        for d in range(len(self.conv_blocks_context) - 1):
            # print(f'------d{d}---------len(self.conv_blocks_context){len(self.conv_blocks_context) - 1}')
            # print('-------- in loop Generic Unet forward ----------')
            # GPUtil.showUtilization()
            # print(len(self.conv_blocks_context),'----conv_blocks_context---')
            x = self.conv_blocks_context[d](x)
            assert torch.isfinite(x).all()
            # print('x.max',x.max(),'x.min',x.min())
            # print('unet line 2260 sum(x)',torch.sum(x))
            # print('x shape in line 2244',x.shape)
            skips.append(x)
            ...
            if hs:
                ...
                h = self.conv_trans_blocks_a[d](hs.pop(0))
                assert torch.isfinite(h).all()
                # print('-----len hs---------', len(hs))
                # print('h in line 2348',h.shape)

                # h = self.ffparser[d](h)
                ha = self.conv_trans_blocks_b[d](h)
                # print('ha in line 2352',ha.shape)
                ...
                if self.train_dim ==3:
                    ...
                    hb = th.mean(h,(2,3))
                    hb = hb[:,:,None,None]
                    # print('hb in line 2360',hb.shape)
                else:
                    print('Generic_UNet forward train_dim not in [2,3], ERROR')
                assert torch.isfinite(hb).all()            
                x = x * ha * hb
                    # del ha, hb
        # print('-------- after loop Generic Unet forward ----------')
        # GPUtil.showUtilization()
        x = self.conv_blocks_context[-1](x)
        # print('unet line 2322 sum(x)',torch.sum(x))
        emb = conv_nd(self.train_dim, x.size(1), 192, 1).to(device = x.device)(x)
        
        # print('len(self.tu)',len(self.tu))
        for u in range(len(self.tu)):
            # print('x shape before',x.shape)
            x = self.tu[u](x)
            # print('x shape after',x.shape)
            # print('skips shape',skips[-1].shape)
            x = th.cat((x, skips.pop(-1)), dim=1)
            x = self.conv_blocks_localization[u](x)
            # print('in unet line 2344 sum(x)',torch.sum(x))
            if self._deep_supervision:
                # print('----deep supervision------')
                pred_outputs.append(self.final_nonlin(self.pred_outputs[u](x)))
        # del skips
        # print('-------- before return Generic Unet forward ----------')
        # GPUtil.showUtilization()
        if not pred_outputs:
            x = self.final_nonlin(self.pred_outputs[0](x))
        # print('in unet line 2352 sum(x)',torch.sum(x))
        # if self._deep_supervision and self.do_ds:
        #     return tuple([pred_outputs[-1]] + [i(j) for i, j in
        #                                       zip(list(self.upscale_logits_ops)[::-1], pred_outputs[:-1][::-1])])
        # else:
        return emb, x
